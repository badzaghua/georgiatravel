...
class UploadsController extends Controller
{
    ...
    public function get_file($hash, $name)
    {
        ...
        if($upload->public || Entrust::hasRole('SUPER_ADMIN') || Auth::user()->id == $upload->user_id) {
            ...
            $path = $upload->path;
            $basepath = base_path();
            $newpath = "$basepath/storage/uploads";
            $path = preg_replace("#^(.*)/storage/uploads#",$newpath,$path);
            ...
            if(isset($size)) {
                ...
                $src = '/files/'.$hash.'/'.$name;
                return redirect()->action('InterventionController@fit', ['h' => $size,'w'=>$size,'src'=>$src]);

                //$thumbpath = storage_path("thumbnails/".basename($upload->path)."-".$size."x".$size);
                $thumbpath = $path.'/100x100';
    ...
    public function uploaded_files()
    {
		if(Module::hasAccess("Uploads", "view")) {
			...
			if(Entrust::hasRole('SUPER_ADMIN')) {
				$uploads = Upload::orderBy('id','desc')->limit(50)->get();
			} else {
